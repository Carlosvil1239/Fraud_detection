...
struct StateTable {
    ...
    std::size_t ensure_state(const std::string& state) {
        const auto it = index.find(state);
        if (it != index.end()) {
            return it->second;
        }
...
fd::model::MarkovModel train_from_dataset_tbb(
        ...
        while (true) {
            ...
            for_each_record_in_buffer(
                ...
                [&](bool has_comma, std::string_view entity_id, std::string_view record) {
                    ...
                    if (entity_to_id.find(entity_id) == entity_to_id.end()) {
    ...
    const std::uint32_t invalid_state = std::numeric_limits<std::uint32_t>::max();
    ...
    // Second pass: count transitions inside blocks in parallel.
    {
        ...
        while (true) {
            ...
            for (std::size_t block_idx = 0; block_idx < results.size(); ++block_idx) {
                for (const auto& [entity_num, info] : results[block_idx].boundaries) {
                    const std::uint32_t prev_state = carry_last_state[entity_num];
    ...
    return fd::model::MarkovModel(state_table.states, probs);
}
