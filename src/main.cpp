...
#include "inference/FraudDetector.hpp"
...
int main(int argc, char** argv) {
    try {
        const auto [input_path, model_path, window, threshold, score_type, backend] = parse_args(argc, argv);

        // Load the model before starting the detection loop.
        const fd::model::MarkovModel model = fd::io::read_model_txt(model_path);

        fd::detection::DetectionOptions opt;
        opt.window = window;
        opt.threshold = threshold;
        opt.score_type = score_type;
        opt.state_position = 1;
        opt.backend = backend;

        const auto t0 = std::chrono::steady_clock::now();
        const fd::detection::DetectionResult result =
                fd::detection::detect_from_dataset(input_path, model, opt);
        ...
        std::cout << "Done.\n";
        std::cout << "Backend: "
                  << (backend == fd::detection::Backend::sequential ? "seq" : "tbb")
                  << "\n";
